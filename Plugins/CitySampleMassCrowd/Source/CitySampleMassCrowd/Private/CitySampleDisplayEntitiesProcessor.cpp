...
void UCitySampleDisplayEntitiesProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& ExecutionContext)
{
	...
	const UMassLODSubsystem& LODSubsystem = ExecutionContext.GetSubsystemChecked<UMassLODSubsystem>();
	...
	ExecutionContext.SetExecutionType(EMassExecutionContextType::Local);

	for (FCitySampleDisplayEntitiesConfig& Config : DisplayConfigs)
	{
		Config.EntityQuery.ForEachEntityChunk(ExecutionContext, [&Config, &ISMInfo, &LocalViewerLocation, &LocalViewerDirection, &CosFOVAngleToDriveVisibility, &MaxRenderDistSquared](FMassExecutionContext& Context)
		{
			...
			for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
			{
				...
				const int32 ISMInfoIndex = Config.StaticMeshDescHandle.ToIndex();
				if (ensureMsgf(ISMInfo.IsValidIndex(ISMInfoIndex), TEXT("Invalid handle index %u for ISMInfo"), ISMInfoIndex))
				{
					if (ISMInfo[ISMInfoIndex].ShouldUseTransformOffset())
					{
						const FTransform& TransformOffset = ISMInfo[ISMInfoIndex].GetTransformOffset();
						const FTransform SMTransform = TransformOffset * TransformFragment.GetTransform();

						ISMInfo[ISMInfoIndex].AddBatchedTransform(Context.GetEntity(EntityIt), SMTransform, SMTransform, 0.0f);
					}
					else
					{
