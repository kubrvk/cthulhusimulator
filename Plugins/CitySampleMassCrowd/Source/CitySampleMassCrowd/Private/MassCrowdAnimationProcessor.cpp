...
void UMassFragmentInitializer_Animation::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& ExecutionContext)
{
	check(World);
	const float GlobalTime = World->GetTimeSeconds();
	EntityQuery.ForEachEntityChunk(ExecutionContext, [this, GlobalTime](FMassExecutionContext& Context)
		{
...
void UMassProcessor_Animation::UpdateAnimationFragmentData(FMassEntityManager& EntityManager, FMassExecutionContext& Context, float GlobalTime, TArray<FMassEntityHandle, TInlineAllocator<32>>& ActorEntities)
{
	...
	for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
	{
		...
		const FMassActorFragment& ActorFragment = ActorInfoList[EntityIt];
		...
		if (!MontageDataList.IsEmpty() && MontageDataList[EntityIt].MontageInstance.SequenceChangedThisFrame())
		{
			AnimationData.GlobalStartTime = GlobalTime - MontageDataList[EntityIt].MontageInstance.GetPositionInSection();
		}
...
void UMassProcessor_Animation::UpdateVertexAnimationState(FMassEntityManager& EntityManager, FMassExecutionContext& Context, float GlobalTime)
{
	...
	for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
	{
		...
		if (Visualization.CurrentRepresentation != EMassRepresentationType::None)
		{
			...
			FMassEntityHandle Entity = Context.GetEntity(EntityIt);
			const UAnimSequence* Sequence = MontageDataList.IsEmpty() ? nullptr : MontageDataList[EntityIt].MontageInstance.GetSequence();
			if (Sequence)
			{
...
void UMassProcessor_Animation::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& ExecutionContext)
{
	check(World);
	...
	
	{
		...
		MontageEntityQuery.ForEachEntityChunk(ExecutionContext, [this, GlobalTime, &ActorEntities, &EntityManager](FMassExecutionContext& Context)
			{
				...
				if (!FMassVisualizationChunkFragment::AreAnyEntitiesVisibleInChunk(Context))
				{
					for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
					{
						...
						FMassMontageFragment& MontageData = MontageDataList[EntityIt];
				...
				else
				{
					...
					for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
					{
						...
						FMassMontageFragment& MontageFragment = MontageDataList[EntityIt];
						...
						if (AdjustedPositionPostAdvance > MontageLength)
						{
							...
							Context.Defer().PushCommand<FMassCommandRemoveFragments<FMassMontageFragment>>(Context.GetEntity(EntityIt));
						}
						else
						{
...

	{
		...
		AnimationEntityQuery_Conditional.ForEachEntityChunk(ExecutionContext, [this, GlobalTime, &ActorEntities, &EntityManager](FMassExecutionContext& Context)
			{
	...
	}
	{
		...
		AnimationEntityQuery_Conditional.ForEachEntityChunk(ExecutionContext, [this, GlobalTime, &EntityManager](FMassExecutionContext& Context)
			{
...

	{
		...
		MontageEntityQuery_Conditional.ForEachEntityChunk(ExecutionContext, [this, &EntityManager](FMassExecutionContext& Context)
			{
				...
				for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
				{
					...
					const FCrowdAnimationFragment& AnimationData = AnimationDataList[EntityIt];
					FTransformFragment& TransformFragment = TransformList[EntityIt];
					FMassMontageFragment& MontageFragment = MontageDataList[EntityIt];
